...
Saves a voice clinical note against a wound case. This endpoint stores the supplied
transcript and/or audio; it does not transcribe audio. Use the transcription endpoint
when speech-to-text is required.

Authentication: bearer token with the `nurse` role.

Path parameters:

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `id` | Positive integer | Yes | Wound case ID accessible to the authenticated nurse. |

The request may use either `application/json` or `multipart/form-data`. At least one
of `transcript`/`text`, an uploaded audio file, or `audio_url` must be supplied.

Body fields:

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `transcript` / `text` | String | Conditional | Existing voice transcript. |
| `audio` | Audio file | Conditional | Audio upload. Accepted aliases: `voice`, `file`, `voice_file`, `voiceFile`, `audio_file`, and `audioFile`. |
| `audio_url` / `audioUrl` | String | Conditional | URL of an already hosted audio recording. |
| `title` | String | No | Note title. Defaults to `Voice Note`. |
| `duration_seconds` / `durationSeconds` | Number | No | Recording duration in seconds. |
| `created_by` / `createdBy` | String | No | Display name or identifier of the creator. |

Uploaded files must have an `audio/*` or `application/octet-stream` MIME type and
must not exceed 25 MB. An uploaded file takes precedence over `audio_url`; its public
URL and file metadata are generated by the server.

JSON example (already hosted audio):
...
| --- | --- | --- |
| `400` | `transcript or audio file is required` | No transcript, uploaded audio, or audio URL was supplied. |
| `400` | `Only audio files are allowed` | Uploaded file has an unsupported MIME type. |
| `400` | Multer file-size message | Uploaded audio is larger than 25 MB. |
| `401` / `403` | Authentication/authorization error | Token is missing, invalid, or is not for a nurse. |
| `404` | `Wound case not found` | Case does not exist or is outside the nurse's patient scope. |
| `500` | `Voice dictation save failed` | Persistence or unexpected server failure. |
...
Generates a structured AI SOAP note from the clinical narrative and the wound case's
current facts, then appends it to `clinical_notes`. The AI is instructed to synthesize
the input, avoid inventing clinical facts, and return all four SOAP sections.
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `text` / `clinical_note` / `clinicalNote` | String | No | Clinical narrative to convert. If omitted, the case's `notes` value is used. |
| `instructions` / `ai_instructions` / `aiInstructions` | String | No | Additional generation guidance. It must not be used to invent patient facts. |
| `created_by` / `createdBy` | String | No | Display name or identifier stored on the generated note. |

Generation also uses the case's wound type, stage, location, etiology, status, pain
score, healing progress, dimensions, existing case notes, and latest measurement.
Client-supplied `subjective`, `objective`, `assessment`, and `plan` fields are not
used; these sections are generated by the AI.
...
```json
{
  ...
  "instructions": "Keep each section concise and mention undocumented findings.",
  "created_by": "Nurse Johnson"
}
```
...
```json
{
  ...
  "clinical_note": {
    ...
    "soap": {
      ...
      "objective": "A left heel wound is documented; other examination findings are not provided.",
      "assessment": "Left heel wound with significant reported pain. Infection findings and interval change are not documented.",
      "plan": "Continue the documented wound-care regimen and reassess pain, wound dimensions, drainage, and signs of infection at follow-up."
    },
...
| `OPENAI_SOAP_MODEL` | No | Preferred SOAP model. |
| `OPENAI_REPORT_MODEL` | No | First fallback when no SOAP model is configured. |

If neither model variable is set, the endpoint falls back to `gpt-4.1-mini`. If a
candidate model fails for a non-authentication/non-rate-limit reason, the next model
is attempted. The model response is schema-constrained and must contain non-empty
`subjective`, `objective`, `assessment`, and `plan` strings.
...
| `401` / `403` | Authentication/authorization error | Token is missing, invalid, or is not for a nurse. |
| `404` | `Wound case not found` | Case does not exist or is outside the nurse's patient scope. |
| `500` | `SOAP note generation failed` | Missing/invalid API configuration, OpenAI failure, incomplete model output, or persistence failure. The response also includes an `error` string. |
